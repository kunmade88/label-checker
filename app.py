...
with st.sidebar:
    ...
    if mode == "Excel vs PDF (성분 검증)":
        ...
        sim_threshold = st.slider("유사도 보정 강도", 0.7, 1.0, 0.85)
    ...
    if excel_file and pdf_file:
        ...
        if st.button("🚀 분석 시작", use_container_width=True):
            ...
            for i, std_name in enumerate(standard_list):
                ...
                status, detected_val = "❌ 오류", "미검출"
                
                for part, raw_part in zip(clean_ocr_parts, ocr_parts):
                    if get_similarity(clean_std, part) > sim_threshold:
                        status, detected_val = "✅ 일치", raw_part
                        break
                
                comparison.append({"No": i+1, "엑셀 기준": std_name, "PDF 실제 검출 내용": detected_val, "상태": status})
...
# --- 모드 2: PDF vs PDF (시각적 차이 분석) ---
    ...
    st.title("🖼️ 문안확인 수정전/후 비교용 테스트 용훈")
    st.info("두 문서의 이미지를 겹쳐서 변경된 부분(디자인, 오타 등)을 빨간 박스로 표시합니다.")
    
    col1, col2 = st.columns(2)
    with col1: f_old = st.file_uploader("📄 원본(Base) 업로드", type=['pdf', 'jpg', 'png'], key="o")
    with col2: f_new = st.file_uploader("📄 수정본(New) 업로드", type=['pdf', 'jpg', 'png'], key="n")
    ...
    if f_old and f_new:
        if st.button("🔍 차이점 분석 실행", use_container_width=True):
            ...
            res_c1.image(img_old_res, caption="이전 버전", use_container_width=True)
            res_c2.image(output, caption="변경 사항 감지 (빨간 박스)", use_container_width=True)